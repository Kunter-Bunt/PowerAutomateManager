...
- The environment has connections across multiple owners and connectors, and at least one Service Principal with an Enterprise Application ID available as a share target.
...
4. **Share (US2, FR-010–012)**: Select several connections → Share → pick one or more Service Principals → confirm → each chosen Service Principal is granted access using its Enterprise Application ID; Teams and individual users are not offered.
