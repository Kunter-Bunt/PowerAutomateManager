...
**Input**: User description: "Next up is a new feature for Connections. Details page show name, Owner and Flows using it. Grouping Options are by Owner and Connector. There are no filters other than the universal search box. Connections can only be shared with Service Principals, not with Teams. The Enterprise Application ID is used for the technical implementation."
...
The toolbar provides a Share action. When invoked on the currently selected connections, the user can choose one or more Service Principals to share those connections with. Teams and individual users are not share targets.
...
**Independent Test**: Select one or more connections, invoke Share, choose one or more Service Principals, and confirm the selected connections are shared with each chosen target.

**Acceptance Scenarios**:

1. **Given** one or more connections are selected, **When** the user clicks Share, **Then** the user can select one or more Service Principals and cannot select Teams or individual users.
2. **Given** Service Principals are chosen, **When** the share is confirmed, **Then** the selected connections are shared with each chosen Service Principal.
...
- What happens when Share is invoked but no Service Principal is chosen? Nothing is shared and the user is prompted to choose at least one Service Principal.
...
- How are Service Principals presented so the user can distinguish them by display name and Enterprise Application ID?
...
- **FR-011**: The Share action MUST let the user select one or more Service Principals as share targets; Teams and individual users MUST NOT be selectable.
...
- **FR-016**: The selectable Service Principals MUST be sourced from the connected environment and identified for sharing by their Enterprise Application ID.
...
- **Share Target**: A Service Principal represented by its Enterprise Application ID. Users and Teams are outside the supported sharing scope.
...
- **SC-003**: A user can share a multi-selected set of connections with one or more Service Principals in a single action, using each target's Enterprise Application ID, and receive per-connection success/failure feedback.
...
- Share presents a picker of Service Principals drawn from the connected environment; Teams and individual users are excluded. The technical share request uses the Service Principal's Enterprise Application ID, and the exact permission level granted follows the platform's standard connection-sharing behavior.
